...
public class GimmickController : MonoBehaviour {
	...
	public GameObject ground;			// The ground Gimmick is currently standing on, or dummyGround if Gimmick is airborne.  Gimmick is a child of this GameObject
	...
	void Update () {
		...
		if (!Object.ReferenceEquals (ground, myRigidbody.gameObject.transform.parent.gameObject))
			handleGroundChange ();
	...
	void OnCollisionEnter2D(Collision2D other) {
		if (other.transform.tag == "Ground" || other.transform.tag == "Star" || other.transform.tag == "StarGround") {
			if (other.contacts.Length >= 2) {
				...
				if ((Mathf.Abs (y1 - y2) < FLOATTOLERANCE) && groundHeight < myHeight) {
					...
					if (yVelBeforeCollisions - myRigidbody.velocity.y < FLOATTOLERANCE) {
	...
	void OnCollisionExit2D(Collision2D other) {
		if (Object.ReferenceEquals (other.transform.gameObject, ground)) {
			isGrounded = false;
			ground = dummyGround;
		}
	}
